    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 gap-y-9 place-items-start">
      {data.map((list) => (
        <div className="flex flex-col relative" key={list.id}>
          ...
          text-white text-base py-1.5 px-4
          ...
          <h2 className="text-base md:text-xl lg:text-2xl font-oswald font-extrabold mb-2">
            {list.title}
          </h2>
          <div className="flex items-center">
            <ClockIcon className="h-4 w-4 lg:h- fill-black/50" />
            <span className="font-urbanist text-sm text-black/50 ml-1.5">
              {list.time}
            </span>
          </div>
        </div>
